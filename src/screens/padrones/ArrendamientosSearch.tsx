...
const ArrendamientoSearch = () => {
  ...
  const searchEmpresa = async () => {
    ...
    response = await getArrendamientos(
      razon_social,
      nombre_comercial,
      RFC,
      currentPage.current,
  ...
  const deleteRow = async rowKey => {
    ...
    let response = await deleteArrendamiento(rowKey);
  ...
  return (
    <>
      <Container>
        ...
        <TopContainer>
          <SearchInput
            placeholderText="Buscar Arrendamiento..."
...
export default ArrendamientoSearch;
